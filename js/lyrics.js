...
export function permittedLyrics(entry, now = Date.now()) {
  ...
  if (
    !rights?.publicDisplay ||
    !rights.staticPublication ||
    !rights.attribution ||
    !["owned", "direct-permission", "public-domain"].includes(rights.kind) ||
    (rights.expiresAt &&
      (!Number.isFinite(Date.parse(rights.expiresAt)) ||
        Date.parse(rights.expiresAt) <= now))
  )
    return null;
  if (
    !Array.isArray(entry.lines) ||
    !entry.lines.length ||
    entry.lines.some((line) => typeof line.text !== "string")
  )
    return null;
  const timed = entry.lines.every(
    (line, i, lines) =>
      Number.isFinite(line.time) &&
      line.time >= 0 &&
      (!i || line.time > lines[i - 1].time),
  );
  return { ...entry, timed };
}
export class LyricsView {
  constructor(root, data) {
    this.root = root;
    this.data = data;
    this.viewport = q("[data-lyrics-viewport]", root);
    this.resume = q("[data-lyrics-resume]", root);
    this.status = q("[data-lyrics-status]", root);
    this.active = -1;
    this.manual = false;
    this.timer = 0;
    this.cleanup = () => {};
  ...
  setTrack(track, clock = null) {
    ...
    if (this.entry?.rights.expiresAt) {
      this.expiryTimer = setTimeout(
        () => this.setTrack(track, clock),
        Math.min(
          2147483647,
          Math.max(1, Date.parse(this.entry.rights.expiresAt) - Date.now()),
        ),
      );
    }
    q("[data-lyrics-title]", this.root).textContent = track.title;
    const link = q("[data-lyrics-link]", this.root);
    const url = safeUrl(track.appleUrl);
    ...
    if (!this.entry) {
      ...
      const wave = el("div", "lyrics-wave");
      wave.setAttribute("aria-hidden", "true");
      for (let i = 0; i < 17; i++) {
        const bar = el("i");
        bar.style.setProperty(
          "--bar",
          String(12 + Math.sin(i * 0.8) ** 2 * 28),
        );
        wave.append(bar);
      }
      ...
      empty.append(
        wave,
        el("p", "", fallback.title || "让声音先抵达。"),
        el("span", "", fallback.description || "歌词可在音乐平台中查看。"),
      );
      this.viewport.append(empty);
    ...
    this.rows = this.entry.lines.map((line, i) => {
      const row = el(
        synced && clock.seek ? "button" : "p",
        "lyric-line",
        line.text || "♪",
      );
      if (row.tagName === "BUTTON") {
        row.type = "button";
        row.setAttribute("aria-label", `跳到：${line.text || "间奏"}`);
        row.addEventListener("click", () => {
          clock.seek(line.time);
          this.follow();
        });
      }
    ...
    if (synced) {
      this.cleanup = clock.subscribe(() => this.update());
      this.update();
    }
  }
  update() {
    ...
    if (this.rows[this.active]) {
      this.rows[this.active].classList.remove("is-current");
      this.rows[this.active].removeAttribute("aria-current");
    }
    this.active = index;
    if (this.rows[index]) {
      this.rows[index].classList.add("is-current");
      this.rows[index].setAttribute("aria-current", "true");
    }
  ...
  center() {
    ...
    const top =
      ...
      (this.viewport.clientHeight - row.offsetHeight) / 2;
...
export function mediaClock(media, offset = 0) {
  return {
    ...
    subscribe(callback) {
      ...
      return () =>
        events.forEach((event) => media.removeEventListener(event, callback));
    },
  };
}
