...
| content/books.json          | 书目身份、摘句、简介、阅读线索与按作者生成的目录 |
...
作者字段自动形成“作者与书目”目录分组；展开作者只浏览该作者作品，不自动跳到第一项。目录可搜索作者／书名，支持当前项、空结果、返回全部、键盘、Escape 和焦点归还。桌面为左书名页／右阅读页；手机为自然单页，正文随内容生长，不使用书页内嵌套滚动。底部“上一项 / 下一项”与方向键切换书目，并清楚显示目标标题。历史 `pages` 字段不驱动当前展示；如未来恢复书内多页，必须与切换书目使用不同的状态、标签和深链接。
...
- 银幕正式画面全部使用真实横版素材；`gallery[]` 逐张记录 `src`、`alt`、`source`、`credit`、原始 `width` / `height` 和 `usage`，`media` 描述首图自然尺寸。主画面保持 16:9，不用裁切竖海报、模糊背景或拉伸制造横图。片名与必要信息在画面外，短评、主创、观看线索和来源收入详情。片内换图、跨作品切换与片目是三种独立导航。
